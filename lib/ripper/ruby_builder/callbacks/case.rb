  ...
  class RubyBuilder < Ripper::SexpBuilder
    module Case
      def on_case(expression, when_block)
        ldelim, rdelim = pop_tokens(:@kw, :value => %w(case end)).reverse
        Ruby::Case.new(expression, when_block, ldelim, rdelim)
      end
      
      def on_when(expression, statements, next_block)
        rdelim = stack_ignore(:@kw, :value => 'end') { pop_token(:@kw, :value => 'then') }
        ldelim = stack_ignore(:@kw, :value => 'end') { pop_token(:@kw, :value => 'when') }
